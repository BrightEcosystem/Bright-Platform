...
1. **Descoberta** — o consumidor final é convidado a entrar na Bright Rewards por uma empresa parceira específica (ex.: ao comprar, ao se cadastrar no estabelecimento, por um cupom/QR code).
2. **Cadastro** — cria conta no aplicativo Bright Rewards, vinculada à empresa parceira que o convidou (podendo, no futuro, se vincular a mais de uma empresa parceira — decisão de arquitetura pendente).
...
4. **Uso recorrente** — a cada compra/interação elegível, acumula cashback e/ou pontos, conforme a Economia do Ecossistema (`040-Economia.md`).
...
8. **Comunidade** — (mecânica listada na visão do ecossistema, `020-Visao.md`) interage com outros consumidores da mesma empresa parceira, se essa funcionalidade for confirmada como parte do escopo — pendente de detalhamento em fase de arquitetura.

## 2. Jornada da empresa (parceira)

1. **Descoberta/decisão** — empresa já cliente da Bright Platform decide habilitar a Bright Rewards como produto adicional (ou empresa nova decide entrar pela Bright Rewards e adota a Bright Platform como base).
...
3. **Convite aos clientes** — convida sua base de clientes existente a entrar no aplicativo Bright Rewards.
4. **Acompanhamento** — acessa, pela Bright Platform (CORE), relatórios de uso do programa: quantos clientes ativos, quanto foi emitido, quanto foi resgatado, ROI percebido.
...
1. **Onboarding de empresa parceira** — acompanha e, se necessário, auxilia a configuração inicial de uma empresa parceira na Bright Rewards.
2. **Monitoramento cross-tenant** — visão agregada (respeitando isolamento por tenant, mesmo modelo de RLS já usado na Bright Platform) de uso, emissão e resgate em todas as empresas parceiras.
