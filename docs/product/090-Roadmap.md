...
**Versão:** 0.3.0 — nomenclatura corrigida por `ADR-002-Arquitetura-do-Ecossistema-Bright.md` (ver `CHANGELOG.md`)
**Parte:** VIII
...
**Objetivo:** validar que uma empresa parceira consegue contratar os módulos de fidelidade e que um consumidor final consegue acumular e resgatar cashback pelo Aplicativo do Consumidor, com transparência total.
...
- Economia da Plataforma (`040-Economia.md`) implementada ao menos nos estados essenciais: `Pendente → Confirmado → Disponível → Resgatado/Expirado/Estornado`.
...
- Marketplace de Benefícios (`070-Integracoes.md §4`), com modelo comercial já decidido pela Direção.
...
- Novas mecânicas de gamificação e novos formatos de campanha além dos listados nesta versão — dentro do escopo do programa de fidelidade (`020-Visao.md §5`), não novos produtos fora dele.
...
| V3 | Receita da Bright por empresa parceira (modelo de monetização, `040-Economia.md §1`) | Volume transacionado no Marketplace de Benefícios |
...
4. Modelo comercial do Marketplace de Benefícios — `070-Integracoes.md §4`.
...
8. ~~Modelagem técnica do novo tipo de usuário (consumidor final)~~ — decisão de alto nível já tomada em `ADR-002-Arquitetura-do-Ecossistema-Bright.md` (identidade única, mesmo mecanismo de autenticação; o consumidor não usa `tenant_memberships` como vínculo final). O desenho técnico exato do vínculo (tabela/RLS) é o objeto da fase `IDENT-001 — Modelo de Identidade do Consumidor e Revisão de RLS`, ainda não iniciada.
