...
## *(em andamento)* — docs: renomeia o projeto para Orbe Flow - Cashback (ADR-003)

A Direção determinou que o projeto passa a se chamar **Orbe Flow - Cashback**. Confirmado por busca completa no repositório (código, docs e histórico) que nenhum conteúdo de gestão de frotas jamais existiu neste projeto — sempre foi, exclusivamente, a plataforma de fidelidade/cashback/gamificação. A arquitetura técnica congelada em `ADR-002` (três camadas, identidade única, Marketplace de Benefícios) não muda — apenas o nome comercial. Atualizados como "branding vivo": `README.md`, `package.json` (`name: orbe-flow-cashback`), `CLAUDE.md` e os títulos de `PROJECT-ROADMAP.md`/`PROJECT-CHECKLIST.md`. Por decisão explícita da Direção, o histórico (este `CHANGELOG.md`, relatórios de fases já concluídas, `ADR-001`/`ADR-002`, e os documentos de arquitetura já congelados — `ARCH-001`, `IDENT-001`, `DATA-001`, `UX-001`, `DS-001`) **não foi reescrito**, permanecendo com o nome "Bright" como registro histórico das decisões já tomadas. Renomeação do repositório GitHub, do projeto Vercel e do projeto Supabase **autorizada pela Direção mas não executada nesta fase** — sem ferramenta disponível no ambiente de execução para isso; ver `docs/decisions/ADR-003-Renomeacao-para-Orbe-Flow-Cashback.md` para o passo a passo manual.

## *(em andamento)* — feat: implementa autenticacao real do consumidor (CORE-002.2)

Substituída a autenticação simulada (`localStorage`) do Aplicativo do Consumidor por Supabase Auth real, autorizada pela Direção com escopo obrigatório de 10 itens e 18 critérios de aceite. Implementados: login/cadastro real (`src/services/consumer-auth/actions.ts`), logout, recuperação e redefinição de senha (`/cliente/esqueci-senha`, `/cliente/redefinir-senha` — rotas técnicas novas, fora do mapa de 12 telas congelado de `UX-001`), e proteção de rotas movida para o servidor (`src/proxy.ts`, `auth.getUser()` a cada requisição — não mais um gate client-side). `ConsumerSessionProvider` reescrito para refletir a sessão real (sem localStorage, sem redirecionamento — isso é responsabilidade do middleware). Confirmado que a identidade única (`IDENT-001`) permite a mesma sessão acessar Retaguarda e Aplicativo do Consumidor sem loop, e que logout em qualquer um dos dois encerra a sessão nos dois (comportamento esperado). **18/18 critérios de aceite aprovados** (ver `docs/reports/CORE-002.2-Relatorio.md`), incluindo bloqueio de open redirect, mensagens genéricas sem enumeração de e-mail, e revalidação de sessão pelo servidor mesmo após o botão "voltar" do navegador. Nenhuma migration criada, nenhum mock de carteira/cashback/missões/benefícios alterado, nenhuma alteração em RLS/funções de `CORE-002.1`. Dados fictícios de teste 100% removidos.
