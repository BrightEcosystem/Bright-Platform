...
| 🔄 | 02/08/2026 | — | CORE-002 | Evolução do Core — Integração Real | — | CORE-002.1 (Revisão Final do Schema e da RLS) — plano técnico revisado v0.2.0 com os 14 ajustes exigidos pela Direção — aguardando nova autorização antes de aplicar migrations |
...
- `CORE-002` (em andamento): liberada pelo gate. Objetivo — transformar o Aplicativo do Consumidor de prova de conceito mockada em aplicação integrada ao Core: autenticação real do consumidor, Conta Fidelidade real (`IDENT-001`), dados reais do Supabase, carteira/saldo/cashback reais, remoção gradual dos mocks. **Fora de escopo nesta fase:** roleta, raspadinha, baús, missões, ranking, XP, campanhas automáticas, notificações, marketplace operacional — permanecem desacoplados até a base de identidade e carteira estar operacional. Não pode alterar a arquitetura já congelada sem ADR. **Análise inicial (v0.1.0) revisada pela Direção** — 14 ajustes obrigatórios exigidos antes de qualquer migration, formalizados como sub-fase **CORE-002.1 — Revisão Final do Schema e da RLS**. **Plano técnico v0.2.0** (`docs/architecture/CORE-002-Plano-Tecnico.md`) incorpora todos os 14 ajustes: ledger unificado por `asset_type`, natureza crédito/débito com valor sempre positivo, chave de idempotência obrigatória, saldo oficial declarado como derivado do ledger (cache nunca é fonte de verdade), RLS completa por operação (SELECT/INSERT/UPDATE/DELETE) e por ator (consumidor/colaborador/`platform.admin`/servidor), nova função `has_permission` para granularidade de permissão em RLS, criação de lançamento exclusivamente via função `security definer` (`criar_lancamento`), lista de índices obrigatórios, estratégia de rollback. **Aguardando nova autorização explícita da Direção antes de aplicar qualquer migration ao Supabase real.**
