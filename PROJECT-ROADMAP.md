...
| 🔄 | 02/08/2026 | — | CORE-002 | Evolução do Core — Integração Real | — | Análise inicial concluída (`docs/architecture/CORE-002-Plano-Tecnico.md`) — aguardando confirmação da Direção antes de aplicar migrations |
...
- `CORE-002` (em andamento): liberada pelo gate. Objetivo — transformar o Aplicativo do Consumidor de prova de conceito mockada em aplicação integrada ao Core: autenticação real do consumidor, Conta Fidelidade real (`IDENT-001`), dados reais do Supabase, carteira/saldo/cashback reais, remoção gradual dos mocks. **Fora de escopo nesta fase:** roleta, raspadinha, baús, missões, ranking, XP, campanhas automáticas, notificações, marketplace operacional — permanecem desacoplados até a base de identidade e carteira estar operacional. Não pode alterar a arquitetura já congelada sem ADR. **Análise inicial concluída** (`docs/architecture/CORE-002-Plano-Tecnico.md`): modelagem física de `contas_fidelidade`/`lancamentos`, RLS mapeada 1:1 da Matriz Oficial (`IDENT-001 §7`), estratégia de autenticação real do consumidor, plano de execução em 5 sub-fases. **Aguardando confirmação da Direção antes de aplicar qualquer migration ao Supabase real.**
