...
- Interação por clique (botão e link) não pôde ser confirmada por gesto de clique real nem em `APP-001` nem em `HOM-001` — testado em dois motores de navegador (local e produção); lógica comprovada correta por invocação direta do handler (`hydrated: true`); recomendável confirmar em dispositivo físico real antes de considerar 100% validado (severidade Baixa, `docs/reports/HOM-001-Relatorio.md §3`)
