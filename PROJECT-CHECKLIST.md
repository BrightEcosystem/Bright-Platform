...
- [ ] **CORE-002** — Evolução do Core — Integração Real: autenticação real do consumidor, Conta Fidelidade real (`IDENT-001`), dados reais do Supabase, carteira/saldo/cashback reais, remoção gradual dos mocks — **análise inicial concluída** (`docs/architecture/CORE-002-Plano-Tecnico.md`: modelagem física, RLS, estratégia de auth, 5 sub-fases) — **aguardando confirmação da Direção antes de aplicar migrations**. Fora de escopo nesta fase: gamificação (roleta, raspadinha, baús, missões, ranking, XP), campanhas automáticas, notificações, marketplace operacional.
